    ...
    IResponder<IGuildRoleCreate>
{
    ...
    public Task<Result> RespondAsync(IGuildRoleCreate gatewayEvent, CancellationToken ct = new())
    {
    ...
    public Task<Result> RespondAsync(IGuildRoleDelete gatewayEvent, CancellationToken ct = new())
    {
        var guildRoles = _discordCache.GetGuildRoles(gatewayEvent.GuildID).Where(x => x.ID != gatewayEvent.RoleID)
            .ToList();
        _discordCache.SetGuildRoles(gatewayEvent.GuildID, guildRoles);
    ...
    public Task<Result> RespondAsync(IGuildRoleUpdate gatewayEvent, CancellationToken ct = new())
    {
        var guildRoles = _discordCache.GetGuildRoles(gatewayEvent.GuildID).Where(x => x.ID != gatewayEvent.Role.ID)
            .ToList();
        guildRoles.Add(gatewayEvent.Role);
        _discordCache.SetGuildRoles(gatewayEvent.GuildID, guildRoles);
