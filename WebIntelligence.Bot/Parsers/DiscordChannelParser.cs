...
namespace WebIntelligence.Bot.Parsers;

public record ChannelParsingResults(IReadOnlyList<IChannel> Channels);

public record ApproximateChannelParsingResults
    (IReadOnlyList<IChannel> Channels, Dictionary<IChannel, int> Scores) : ChannelParsingResults(Channels);
...
public class DiscordChannelParser : AbstractTypeParser<IChannel>
{
    ...
    public Result<ChannelParsingResults> TryParseChannelFromName(string value, IReadOnlyList<IChannel> channelList)
    {
        var channels = channelList
            .Where(x => string.Equals(x.Name.Value!.Replace("#", ""), value.Replace("#", ""),
    ...
    public Result<ApproximateChannelParsingResults> TryParseChannelFromNameLevenshtein(string value,
        IReadOnlyList<IChannel> channelList)
    {
        var channels = channelList
            .Select(x => new
            {
