...
public class DiscordCache
{
    ...
    public void SetGuildSelfMember(Snowflake guildId, IGuildMember guildMember) =>
        _appCache.Add($"{guildId.Value}/SelfMember", guildMember, _neverExpireCacheEntryOptions);
    ...
    public void SetGuildRoles(Snowflake guildId, IReadOnlyList<IRole> roles) =>
        _appCache.Add($"{guildId.Value}/Roles", roles, _neverExpireCacheEntryOptions);

    public IReadOnlyList<IChannel> GetGuildChannels(ulong guildId) =>
        _appCache.Get<IReadOnlyList<IChannel>>($"{guildId}/Channels");

    public IReadOnlyList<IChannel> GetGuildChannels(Snowflake guildId) => GetGuildChannels(guildId.Value);

    public void SetGuildChannels(Snowflake guildId, IReadOnlyList<IChannel> channels) =>
        _appCache.Add($"{guildId.Value}/Channels", channels, _neverExpireCacheEntryOptions);
    ...
    public void SetEveryoneRole(Snowflake guildId, IRole role) =>
        _appCache.Add($"{guildId.Value}/EveryoneRole", role, _neverExpireCacheEntryOptions);
    ...
    public void SetGuildMember(ulong discordGuildId, ulong discordUserId, IGuildMember member) =>
        _appCache.Add($"{nameof(GetGuildMember)}/{discordGuildId}/{discordUserId}",
            Result<IGuildMember>.FromSuccess(member), TimeSpan.FromMinutes(5));
}