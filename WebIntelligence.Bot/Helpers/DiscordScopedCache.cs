...
public class DiscordScopedCache : DiscordCache
{
    ...
    public Task<Result<IGuildMember>> GetGuildMember(ulong discordUserId) =>
        GetGuildMember(_commandContext.GuildID.Value.Value, discordUserId);

    public async Task<Result<IGuildMember>> GetInvokingGuildMember() =>
        await GetGuildMember(_commandContext.GuildID.Value.Value, _commandContext.User.ID.Value);
}